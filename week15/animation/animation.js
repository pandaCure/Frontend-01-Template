export class Timeline {
  constructor() {
    ...
    this.tick = () => {
      ...
      if (animations.length) {
        // 没有正在进行的animation时候，不再requestAnimationFrame
  ...
  resume() {
    // 此时多次点击resume就会出错，因此需要用state去管理
  ...
  add(animation, addTime) {
    // 加animation的时候需要知道这个animation是从多长时间之后开始执行，还是从0（马上）开始
    this.animations.push(animation)
    animation.finished = false
    if (this.state === 'playing') {
      animation.addTime =
        addTime !== void 0 ? addTime : Date.now() - this.startTime // addTime是个时间段(也与progress动画的进程有关的)，要么是从加入进来立即执行，要么是传入个addTime，注意与deley的区分，deley是在每个animation自己身上的，自己决定
    } else {
...
export class Animation {
  ...
  valueFromProgression(progression) {
    // 将value的计算逻辑移入anmation类来
    return this.start + progression * (this.end - this.start)
  }
}

export class ColorAnimation {
  // 这时候start/end可能是个rgb的值
  constructor(
    object,
    property,
    start,
